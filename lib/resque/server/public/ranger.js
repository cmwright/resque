  ...
  var poll_start = function(el) {
    ...
    setInterval(function() {
      $.ajax({dataType: 'text', type: 'get', url: href, success: function(data) {
        ...
        if(last_diff){
          var current_queue_length = parseInt($(".queues .size:first").text());
          var this_diff = current_queue_length - last_diff;
          previous_diffs.push(this_diff);
          $(".queues .size:first").text( + "(" + calculate_array_average(previous_diffs) + ")");
          old_size = current_queue_length;
        }else{
          last_diff = parseInt($(".queues .size:first").text());
        }
      }})
    }, poll_interval * 1000)
...
var calculate_array_average = function(ary){
  var sum = 0
